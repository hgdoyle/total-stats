...
with col1:
    csv_files = st.file_uploader(
        """Upload your CSV files (Download each sheet from the regular season stats spreadsheet as a .csv file and upload them here. The format should be as follows: week-1.csv, week-2.csv and so on))""",
        type="csv",
...
if st.button("Run Script", key="main_run_button"):
    if not (csv_files and names_file):
        st.warning("Please upload both your CSV files and names.txt before running.")
    else:
            ...
            try:
                ...
                if os.path.exists(output_path):
                    with open(output_path, "rb") as f:
                        st.download_button(
                            label="📥 Download Total.csv",          # ← fixed label to match OUTPUT_FILE
                            data=f,
                            file_name="Total.csv",
                            mime="text/csv",
                            key="dl_total_main"
                ...
                # Optional: also offer Allstate.csv if it exists
                allstate_path = os.path.join(tmp_dir, "All-TotalStats/Allstate.csv")
                if os.path.exists(allstate_path):
                    with open(allstate_path, "rb") as f:
                        st.download_button(
